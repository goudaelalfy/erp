...
$(document).ready(function() {
	...
	$('#myMenu1 .addFolder').click(function()  {  treeOps.addAccountReq(); });	
	...
	simpleTree = $('.simpleTree').simpleTree({
		...
		//	alert("destination-"+destination.attr('id')+" source-"+source.attr('id')+" pos-"+pos);	
			...
			{				
				...
				//var destination_id= destination.attr('id');
			 	//var structureManagerURL = '../ums/users_rules/update_parent/'+source_id+'/'+destination_id;
		...
		afterContextMenu: function(element, event)
		{
			...
			if (className.indexOf('doc') >= 0) {
				...
				$('#myMenu2').css('top',event.pageY-110).css('left',event.pageX-230).show();
			}
			else {
				//Added By Gouda.
				$('#myMenu2').hide();
				...
				$('#myMenu1').css('top',event.pageY-110).css('left',event.pageX-230).show();
			}
