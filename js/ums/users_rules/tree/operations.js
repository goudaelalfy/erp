		...
		$('#inputText').bind("keypress", 
			 ...
			 {												
				...
				{	
					...
					var parent_id = $('#inputText').parent().parent().parent().attr('id');
					var params = encodeURI("action=insertElement"+"&name="+name+"&parent_id="+parent_id+"&slave="+slave);
...
////////////////////////////////////////////////////// By Gouda ///////////////////
	...
	{    
		...
		var parent_id =	treeOps.trGetSelected().attr('id');
		
		var newwindow=window.open('../ums/users_rules/go_form/add/'+parent_id,'Add Users Rule','width=600px,height=450px,toolbar=no,sizeable=0,  directories=no,status=yes,menubar=no,scrollbars=yes,copyhistory=yes');
...
//////////////////////////////////////////////////////////////////////////////////
	...
	this.updateElementNameReq = function()
	{
		...
		var id = treeOps.trGetSelected().attr('id');	
		
		var newwindow=window.open('../ums/users_rules/go_form/edit/'+id,'Edit Users Rule','width=600px,height=450px,toolbar=no, sizeable=0,  directories=no,status=yes,menubar=no,scrollbars=yes,copyhistory=yes');
		if (window.focus) {newwindow.focus();}
...
//////////////////////////////////////////////////////////////////////////////////
	...
	{	
		...
		{
			treeOps.treeBusy = true;
			var parent_id = treeOps.trGetSelected().parent().parent().attr('id');
		 	var params = "action=deleteElement&elementId="+treeOps.trGetSelected().attr('id')
		 				 + "&parent_id=" + parent_id;
		 	
		 	var id = treeOps.trGetSelected().attr('id');
		 	var structureManagerURL = '../ums/users_rules/delete/'+id;
...
*******************************************************/
	this.trUpdateElementName = function(result)
	{
		...
		$('ul.ajax>li.doc-last', '#' + elementId).attr('id', info.elementId).html("{url:"+ structureManagerURL +"?action=getElementList&parent_id="+ info.elementId +"}");
