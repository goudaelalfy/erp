...
function validate_form(lang) {
	 ...
	 {
		 if(lang=='english')
			 alert('Rule code reduired');
		 else
			 alert('كود القاعدة مطلوب');
	 ...
	 {
		 if(lang=='english')
			 alert('Rule name reduired');
		 else
			 alert('اسم القاعدة مطلوب');
...
{
  document.frm_process_data.submit(); 
  //window.close();
  //window.opener.location.reload(false);
}


function on_select(id,node_name)
{
  window.close();
  window.opener.document.getElementById('hdn_user_rule').value=id;
  window.opener.document.getElementById('txt_user_rule').value=node_name;
}

/*
function reload_tree(url)
{
	 var strURL=url;
	    ...
	    req.onreadystatechange = function()
	    {
	      if (req.readyState == 4) // only if "OK"
	      {
	        ...
	        {alert(req.responseText);
	          document.getElementById('annualWizard').innerHTML=req.responseText;
	        } else {
